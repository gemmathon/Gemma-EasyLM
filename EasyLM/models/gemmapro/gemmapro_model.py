...
from .configuration_gemmapro import GemmaProConfig
...
GEMMA_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`GemmaProConfig`]): Model configuration class with all the parameters of the model.
...
class FlaxGemmaAttention(nn.Module):
    ...
    def __call__(
        ...
        if attention_mask is None:
            ...
            attention_mask = jnp.ones((batch_size, hidden_states.shape[1])) # FIXME: 왜 attention mask가 None이지??
...
class FlaxGemmaModule(nn.Module):
    ...
    def setup(self):
        ...
        self.freq_cis = create_sinusoidal_positions(
            self.config.max_position_embeddings, self.config.head_dim
        )
